...
**新增**
- OG / Twitter 分享卡片，链接转发到微信 / QQ / X 不再是裸 URL（`og:image` 预留启用说明）
- 漂流瓶移动端支持：长按 0.8s 捞起漂流瓶读评论（桌面悬停交互不变）
- 鼠标拖尾粒子全面重绘：春桃花瓣（含五瓣小桃花点缀）/ 夏蒲公英乘风飞散 / 秋银杏翻飞 / 冬六臂枝晶雪花 / 夜四芒星尘
- 夜间流星点缀；切换主题时拖尾粒子软过渡，不再瞬间清空
- 主题选择自动记忆（刷新 / 重开不丢）；首次进站昼夜跟随系统偏好、四季按真实月份
- 想法流「显示更多」分页、搜索无结果提示；列表加载失败可原地重试
- 相册缩略图：网格加载 ~400px 缩略图，灯箱才加载原图（旧照片自动回退）
- 内容缓存（TTL 10 分钟）与漂流瓶评论池复用，明显减少 GitHub API 请求

**优化**
- Markdown 渲染链接入 DOMPurify：先净化再插入，任一依赖缺失整体降级为安全纯文本
- GitHub API 限流自动退避重试；列表自动翻页突破单页 100 条上限；点赞省一次往返
- 四个 CDN 脚本改为 defer 加载，并预连接 API / CDN 域名，首屏更早可交互
- marked 版本锁定（v18.0.12），避免上游破坏性更新静默破坏渲染
- 主题面板在移动端可点边条展开（桌面悬停行为不变）
- 想法流正文 HTML 结构语义化（`div.md` 替代浏览器容错的嵌套 `<p>`）

**修复与适配**
- 长链接 / 长单词 / 超宽图片撑破布局
- 窄屏下时间线圆点与竖线错位
- iOS 输入框聚焦时自动放大整页（字号固定 16px）
- 相册大图加载失败显示占位提示而非空白
- 键盘可达性：为 `outline: none` 的控件补 `:focus-visible` 焦点框

### 日常更新

> 版本发布之间的日常推送（不升版本号的内部优化 / 小更新）记录在这里，格式：`#### YYYY-MM-DD` + 列点。

#### 2026-09-14

- 新增三种漂流 / 悬停几何体：圆锥、正八面体、六棱柱，与原立方体 / 三棱锥 / 圆柱体共 6 种等概率出现；半浸没、倾斜、悬停捞起、主题配色与碰撞检测全部沿用既有机制
- 想法流卡片、相册照片进入视口时淡入上移（交错入场，尊重 `prefers-reduced-motion`）
- 首屏名字「你的名字」改为手绘花体连笔 SVG，加载后自动描边写出，颜色跟随主题
- 头像双层交互：常态为线框立方体，鼠标进入时切换为绕自身重心自转的彭罗斯三角（不可能三角），带轻微跟随位移；头像圆形区域内不再触发海洋悬停提几何体
- 修复：彭罗斯三角边缘完整不裁切、悬停时原立方体正确淡出、旋转中心对齐三角形重心
