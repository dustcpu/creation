...
**视觉与主题**
- 昼夜 × 四季共 8 套主题配色，左侧面板悬停展开切换；四季秒切、昼夜 1s 平滑过渡
...
- **Markdown 渲染依赖 CDN**：`marked.js` 走 CDN 引入（已放在 `</body>` 前，不阻塞首屏）。若部署环境访问不到该 CDN，想法流与评论会自动回退成「换行即换行」的纯文本渲染——页面不会报错，但标题、代码块、表格、引用等样式会失效。需要离线或内网使用时，把 `marked.min.js` 下载到本地并改成相对路径即可。
