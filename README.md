...
**新增**
- 六种漂流 / 悬停几何体：在立方体、三棱锥、圆柱体之外新增**圆锥、正八面体、六棱柱**，六种等概率出现；半浸没、倾斜、悬停捞起、主题配色与碰撞检测全部沿用既有机制
- 想法流卡片、相册照片进入视口时淡入上移（交错入场，尊重 `prefers-reduced-motion`；老浏览器直接显示）
- 首屏名字改为手绘花体连笔 SVG，加载后自动描边写出，句点最后淡入，颜色跟随主题
- 头像双层交互：常态为线框立方体，鼠标进入切换为绕自身重心自转的**彭罗斯三角**（不可能三角），带轻微跟随位移
- 彭罗斯三角随自转产生明暗：以左上为固定光源，三条臂按方位角实时着色（迎光白 → 中间色 → 背光深，平滑渐变），五套主题各配一组颜色

**优化**
- 头像圆形区域与海洋悬停显式隔离：圆内不再误触发「提几何体」，圆外海洋交互不变
- 彭罗斯三角改为绕**三角形重心**自转（而非画布中心），转起来不再像画圈；画布以重心为中心留足外接半径，任意角度完整不裁切

**修复与适配**
- 正八面体上下顶点参数修正为与立方体同一投影下的 ±A，不再是偏扁的双金字塔
- 彭罗斯三角重画：由只能画出普通三角框的构件旋转法，改为亮 / 中 / 暗三面多边形坐标，才构成不可能三角的错觉
- 悬停时原线框立方体正确淡出；修掉彭罗斯高光层被内外两层容器重复旋转导致的重影
